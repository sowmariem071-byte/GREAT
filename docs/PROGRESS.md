...
## 2026-07-10 00:48 - 公网部署数据库链路诊断
- 目标：根据公网发布要求，排查 Vercel 生产环境 `/api/auth/login` 返回 500 的根因，确认是环境变量、数据库连接、迁移还是 seed 问题。
- 发现：
  - 公网 `/login` 返回 200，页面可正常加载。
  - 公网 `POST /api/auth/login` 无论正确账号还是错误账号都返回 500，说明问题发生在服务端登录接口访问数据库之前或过程中。
  - 当前代码没有前端 Supabase client，数据库通过 Prisma 服务端直连 PostgreSQL，生产核心环境变量是 `DATABASE_URL`。
  - 本地 `.env` 仍指向 localhost，不能代表 Vercel 生产环境。
- 变更：
  - 临时新增只读诊断接口 `/api/health`，用于返回数据库连接是否成功、用户/目标数量、关键环境变量是否存在，以及 Prisma 错误码。
  - 诊断接口不返回任何密钥或完整连接串。
- 涉及文件：
...
- 变更：
  - 表单标题从 `Create Account` 改为“欢迎登录”。
  - 输入框占位从 `Email`、`Password` 改为“账号”“密码”。
  - 错误提示改为“账号或密码错误，请重新输入。”。
  - 主按钮从 `Register` 改为“登录”，次按钮从 `Back` 改为“返回”。
  - 底部 checkbox 说明改为中文，并将 `Terms & Conditions`、`Privacy Policy` 改为“服务条款”“隐私政策”。
  - 同步更新需求文档和开发计划文档，明确公网登录页表单使用中文文案。
- 涉及文件：
  - `src/app/login/page.tsx`
  - `docs/REQUIREMENTS.md`
  - `docs/PLAN.md`
...
- 变更：
  - 登录页顶部品牌文字桌面端从 18px 放大到 36px，移动端调整为 22px。
  - 顶部栏高度从 40px 调整为 56px，避免放大后的品牌文字被压缩。
  - 官网入口按钮改为淡粉渐变背景、白色文字、淡粉描边和轻阴影。
  - 登录主体高度公式从 `calc(200svh - 266px)` 调整为 `calc(200svh - 298px)`，为放大后的顶部栏预留空间。
- 涉及文件：
  - `src/app/app.css`
  - `docs/PROGRESS.md`
- 验证：
...
- 变更：
  - 登录页新增顶部 `login-topbar`，左侧显示“母婴事业部”。
  - 顶部右侧新增“进入【蒸蒸爸】官网”链接，跳转到 `https://ihmzoectzzq0.meoo.fun/`，新窗口打开。
  - 顶部入口使用轻量描边圆角按钮，保持参考图里的简洁官网感。
  - 登录主体宽高公式从 `calc(200svh - 186px)` 调整为 `calc(200svh - 266px)`，为 40px 顶栏和 28px 间距预留高度，避免新增顶部后卡片顶到底。
  - 补充移动端顶部品牌栏字号和官网按钮尺寸适配。
  - 同步更新需求文档和开发计划文档，明确登录页包含轻量顶部品牌栏。
- 涉及文件：
  - `src/app/login/page.tsx`
  - `src/app/app.css`
  - `docs/REQUIREMENTS.md`
